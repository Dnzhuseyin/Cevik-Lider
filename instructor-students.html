...
<head>
    ...
    <title>Öğrenci Yönetimi - EduPlatform Admin</title>
...
<body class="bg-gray-50 min-h-screen">
    ...
    <div id="student-modal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 hidden">
        <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-2/3 shadow-lg rounded-md bg-white max-h-[90vh] overflow-y-auto">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-2xl font-bold text-gray-900">Öğrenci Detayları</h3>
    ...
    <script>
        ...
        async function loadStudents() {
            try {
                    ...
                    try {
                        ...
                        if (email && !email.includes('koordinator') && !email.includes('instructor')) {
                            ...
                            studentMap.set(email, {
                                ...
                                school: profile.school || 'EduPlatform Üniversitesi',
                ...
                console.log(`✅ ${allStudents.length} gerçek öğrenci yüklendi`);
        ...
        async function createSampleStudents() {
            ...
            console.log('ℹ️ Default öğrenci oluşturma atlandı - sadece gerçek kayıtlı kullanıcılar gösterilecek');
        }
        ...
        async function viewStudentDetail(studentId) {
            console.log('🔍 viewStudentDetail çağrıldı:', studentId);
            console.log('📊 allStudents:', allStudents.length, 'öğrenci');
            console.log('📊 allStudents array:', allStudents);
            
            // Force alert to check if function is called
            alert(`🔍 viewStudentDetail çağrıldı! ID: ${studentId}, Öğrenci sayısı: ${allStudents.length}`);
            ...
            if (!student) {
                ...
                console.error('❌ Aranan ID:', studentId, 'Tip:', typeof studentId);
                alert('❌ Öğrenci bulunamadı: ' + studentId);
                return;
            }

            console.log('✅ Öğrenci bulundu:', student.name);
            alert('✅ Öğrenci bulundu: ' + student.name);
            ...
            if (!content) {
                console.error('❌ Content bulunamadı');
                alert('❌ Content element bulunamadı!');
                return;
            }
            ...
            content.innerHTML = `
                <div class="flex items-center justify-center py-12">
                    ...
                    <span class="text-lg text-gray-600">Öğrenci detayları yükleniyor...</span>
                </div>
            `;
            ...
            try {
                ...
                const videoQuestionAnswers = videoQuestionAnswersResult.success ? 
                    videoQuestionAnswersResult.data.filter(a => a.userEmail === student.email) : [];
                
                console.log(`📝 ${videoQuestionAnswers.length} video soru cevabı bulundu`);
                ...
                if (videoQuestionAnswers.length > 0) {
                    videoQuestionAnswersHtml = `
                        <h3 class="text-lg font-semibold mb-3 mt-6">Video Soru Cevapları</h3>
                        <div class="space-y-3">
                            ${videoQuestionAnswers.map(answer => {
                                const video = allCoordinatorVideos.find(v => v.id === answer.videoId);
                                const videoTitle = video ? video.title : 'Bilinmeyen Video';
                                ...
                                return `
                                    <div class="bg-white p-4 rounded-lg border border-gray-200">
                                        <div class="flex justify-between items-start mb-2">
                                            <div class="flex-1">
                                                <h4 class="font-medium text-gray-900">${videoTitle}</h4>
                                                <p class="text-sm text-gray-600 mt-1">${answer.questionText}</p>
                                            </div>
                                            <span class="px-2 py-1 text-xs rounded-full ${statusClass} ml-3">
                                                <i class="${statusIcon} mr-1"></i>${isCorrect ? 'Doğru' : 'Yanlış'}
                                            </span>
                                        </div>
                                        <div class="space-y-2">
                                            <div class="flex items-start">
                                                <span class="text-xs font-medium text-gray-500 w-20">Cevabı:</span>
                                                <span class="text-sm text-gray-900">${answer.studentAnswer}</span>
                                            </div>
                                            ${!isCorrect && answer.correctAnswer ? `
                                                <div class="flex items-start">
                                                    <span class="text-xs font-medium text-gray-500 w-20">Doğru:</span>
                                                    <span class="text-sm text-green-600">${answer.correctAnswer}</span>
                                                </div>
                                            ` : ''}
                                            ${answer.explanation ? `
                                                <div class="flex items-start">
                                                    <span class="text-xs font-medium text-gray-500 w-20">Açıklama:</span>
                                                    <span class="text-sm text-gray-600">${answer.explanation}</span>
                                                </div>
                                            ` : ''}
                                            <div class="flex items-start">
                                                <span class="text-xs font-medium text-gray-500 w-20">Tarih:</span>
                    ...
                    videoQuestionAnswersHtml = `
                        <h3 class="text-lg font-semibold mb-3 mt-6">Video Soru Cevapları</h3>
                        <p class="text-gray-500 text-sm">Henüz video sorularına cevap verilmemiş.</p>
                    `;
                }
            ...
            content.innerHTML = `
                ...
                <div class="mt-6">
                    ...
                    <div class="bg-blue-50 p-4 rounded-lg">
                        ...
                        <p class="text-sm text-gray-700 mt-2">
                            <strong>Toplam İlerleme:</strong> ${student.totalProgress}%
                        </p>
                    </div>
                </div>
                ${videoQuestionAnswersHtml}
            ...
            console.log('Modal açıldı!');
                ...
                content.innerHTML = `
                    <div class="text-center py-12 text-red-600">
                        ...
                        <p class="text-lg">Detaylar yüklenirken bir hata oluştu.</p>
