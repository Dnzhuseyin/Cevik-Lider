...
<head>
    ...
    <title>Modül Detayı - Çevik Lider</title>
            ...
            <header class="mb-8">
                <div class="flex items-center mb-4">
                    ...
                    <div>
                        ...
                        <p class="text-gray-600 mt-2" id="module-description">İçerik yükleniyor...</p>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2">
                    ...
                    <div class="bg-white rounded-xl shadow-sm overflow-hidden mb-6 border border-gray-100">
                        ...
                        <div class="p-6">
                            <h2 class="text-xl font-semibold mb-2" id="video-title">Video Yükleniyor...</h2>
                            <p class="text-gray-600 mb-4" id="video-description">Açıklama yükleniyor...</p>
                    ...
                    <!-- Questions Section - Videonun Altında -->
                    <div class="bg-white rounded-xl shadow-sm p-6 mb-6 border border-gray-100">
                        ...
                        <div id="questions-container">
                            <div class="text-center text-gray-500 py-8">
                                ...
                                <p class="text-sm">Sorular yükleniyor...</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm p-6 mb-6 border border-gray-100">
                        ...
                        <div class="space-y-3" id="video-list">
                            <div class="text-center py-8 text-gray-500">
                                <i class="fas fa-spinner fa-spin text-2xl mb-3"></i>
                                <p class="text-sm">Videolar yükleniyor...</p>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        async function loadModuleContent() {
            ...
            if (moduleParam) {
                ...
                console.log('📍 URL\'den alınan modül ID:', currentModule, 'Tip:', typeof currentModule);
            }
        ...
        async function loadCoordinatorVideos() {
            console.log('🔥 Loading coordinator videos for module:', currentModule);
            
            try {
                // Wait for Firebase to be ready
                if (!window.DB || !window.DB.isFirebaseReady) {
                    console.log('⏳ Firebase hazır değil, bekleniyor...');
                    let waitAttempts = 0;
                    while ((!window.DB || !window.DB.isFirebaseReady) && waitAttempts < 10) {
                        await new Promise(resolve => setTimeout(resolve, 500));
                        waitAttempts++;
                        console.log(`⏳ Firebase bekleme denemesi: ${waitAttempts}/10`);
                    }
                }
                ...
                console.log('🔍 Tüm koordinatör videoları:', coordinatorVideos);
                console.log('🔍 Aranan modül ID:', currentModule, 'Tip:', typeof currentModule);
                ...
                // Filter by current module - tüm olası tipleri kontrol et
                const moduleCoordinatorVideos = coordinatorVideos.filter(video => {
                    ...
                    if (match) {
                        console.log(`✅ Video eşleşti: "${video.title}" (moduleId: ${video.moduleId})`);
                    }
                ...
                console.log(`📊 Modül ${currentModule} için ${allVideos.length} video bulundu`);
                ...
                if (allVideos.length > 0) {
                    ...
                    console.log(`✅ ${videoObjects.length} video yüklendi`);
                } else {
                    console.log('⚠️ No videos found for module', currentModule);
                    if (!moduleData[currentModule]) {
                        moduleData[currentModule] = {
                            title: `Modül ${currentModule}`,
                            description: `Bu modül için henüz video eklenmemiş`,
                            videos: []
                        };
                    } else {
                        moduleData[currentModule].videos = [];
                        moduleData[currentModule].description = `Bu modül için henüz video eklenmemiş`;
                    }
                }
                ...
                if (!moduleData[currentModule]) {
                    moduleData[currentModule] = {
                        title: `Modül ${currentModule}`,
                        description: `Video yükleme hatası`,
                        videos: []
                    };
                } else {
                    moduleData[currentModule].videos = [];
                    moduleData[currentModule].description = `Video yükleme hatası`;
                }
            }
        }
        ...
        async function loadModule() {
            ...
            console.log('📚 Modül bilgisi yüklendi:', module);
            
            if (!module) {
                console.warn('⚠️ Modül bulunamadı, moduleData kontrol ediliyor...');
                // Fallback to moduleData
                const moduleFromData = moduleData[currentModule];
                if (!moduleFromData) {
                    document.getElementById('module-title').innerHTML = '<i class="fas fa-exclamation-triangle text-red-500 mr-3"></i>Modül Bulunamadı';
                    document.getElementById('module-description').textContent = 'Bu modül mevcut değil.';
                    return;
                }
            ...
            const allModuleVideos = moduleData[currentModule]?.videos || [];
            // Filter out deleted videos for students
            const moduleVideos = allModuleVideos.filter(video => video.status !== 'deleted');
        ...
        function showNoVideosMessage() {
            document.getElementById('video-title').textContent = 'Video Mevcut Değil';
            document.getElementById('video-description').textContent = 'Bu modül için henüz video eklenmemiştir.';
            document.getElementById('video-placeholder').innerHTML = `
                <div class="text-center">
                    <i class="fas fa-video-slash text-6xl mb-4 opacity-75"></i>
                    <p class="text-lg">Video Mevcut Değil</p>
                    <p class="text-sm opacity-75 mt-2">Koordinatör tarafından video eklendiğinde burada görünecektir.</p>
                    </div>
                `;
        ...
        async function loadVideoList() {
            ...
            if (!module || !module.videos || module.videos.length === 0) {
                videoList.innerHTML = `
                    <div class="text-center py-8 text-gray-500">
                        ...
                        <p class="text-sm">Bu modül için henüz video eklenmemiştir.</p>
                    </div>
                `;
                return;
            }
            ...
            // Filter out deleted videos for students
            const activeVideos = module.videos.filter(video => video.status !== 'deleted');
                    ...
                    <div class="flex items-start justify-between">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 mb-1">
                                <h4 class="font-medium text-sm truncate ${isActive ? 'text-white' : 'text-gray-900'}">${video.title || 'İsimsiz Video'}</h4>
                                ${isCompleted 
        ...
        function loadVideo(video) {
            ...
            console.log('🎬 Video yükleniyor:', video);
            ...
            if (video.videoUrl && video.videoUrl !== '#') {
                ...
                if (youtubeId) {
                    ...
                    videoPlaceholder.innerHTML = `
                        ...
                        `;
                    console.log('✅ YouTube video yüklendi:', youtubeId);
                    } else {
                    // No valid YouTube ID
                    ...
                    videoPlaceholder.innerHTML = `
                        <div class="text-center">
                            <i class="fas fa-video-slash text-6xl mb-4 opacity-75"></i>
                            <p class="text-lg">Geçersiz Video URL</p>
                            <p class="text-sm opacity-75 mt-2">YouTube video ID'si çıkarılamadı.</p>
                        </div>
                    `;
                    }
                } else {
                ...
                videoPlaceholder.innerHTML = `
                    <div class="text-center">
                        <i class="fas fa-video-slash text-6xl mb-4 opacity-75"></i>
                        <p class="text-lg">Video URL Mevcut Değil</p>
                        <p class="text-sm opacity-75 mt-2">Bu video için henüz URL tanımlanmamıştır.</p>
                    </div>
                `;
            }
            ...
            if (!video.questions || video.questions.length === 0) {
                questionsContainer.innerHTML = `
                    <div class="text-center text-gray-400 py-12">
                        ...
                        <p class="text-lg">Bu video için henüz soru eklenmemiş.</p>
                        <p class="text-sm mt-2">Koordinatör tarafından sorular eklendiğinde burada görünecektir.</p>
                    </div>
                `;
                return;
            }

            console.log(`📝 ${video.questions.length} soru yükleniyor...`);
            ...
            questionsContainer.innerHTML = video.questions.map((q, index) => {
                            ...
                            <div class="flex-1">
                                <div class="flex items-center gap-2 mb-2">
                                    ...
                                    <span class="px-2 py-1 text-xs font-medium rounded-full ${
                                        ...
                                        ${questionType === 'multiple-choice' ? 'Çoktan Seçmeli' :
                                          questionType === 'open-ended' ? 'Açık Uçlu' :
                                          'Boşluk Doldurma'}
                                    </span>
                    </div>
                    ...
                    questionHTML += `
                        <div class="ml-14">
                            ...
                            <button onclick="submitOpenEndedAnswer(${index})" 
                                ...
                                <i class="fas fa-check mr-2"></i>Cevabı Gönder
                            </button>
                                    </div>
                    `;
                }
                                ...
                                ${questionParts.map((part, partIndex) => {
                                    ...
                                    if (partIndex < blankCount) {
                                        html += `<input type="text" 
                                                       ...
                                                       placeholder="Boşluk ${partIndex + 1}" />`;
                                    } else if (partIndex < questionParts.length - 1) {
                                        // If there are more ___ than correctAnswers, show as text
                                        html += `<span class="inline-block mx-2 px-3 py-2 bg-gray-100 rounded-lg text-gray-500">[Boşluk ${partIndex + 1}]</span>`;
                                    }
                                    return html;
                                }).join('')}
                                </div>
                            <p class="text-xs text-gray-500 mt-2 ml-14">${blankCount} boşluk doldurmanız gerekiyor.</p>
                ...
                questionHTML += `
                    ...
                    <i class="fas fa-lightbulb mr-2"></i>
                                <strong>Açıklama:</strong> <span class="explanation-text"></span>
                            </p>
                        </div>
                    </div>
                `;
        ...
        function submitOpenEndedAnswer(questionIndex) {
            ...
            saveQuestionAnswer(questionIndex, question.question, answer, 'Açık uçlu soru', null, question.explanation);
            
            // Show feedback (for now, just show a message)
            alert('Cevabınız kaydedildi. Koordinatör tarafından değerlendirilecektir.');
        ...
        async function submitFillBlankAnswer(questionIndex) {
            ...
            const studentAnswerText = userAnswers.join(', ');
            const correctAnswerText = correctAnswers.join(', ');
            
            // Save fill-blank answer
            saveQuestionAnswer(questionIndex, question.question, studentAnswerText, correctAnswerText, isCorrect, question.explanation);
            ...
            if (isCorrect) {
                if (explanationBox) {
                    ...
                    explanationBox.querySelector('.explanation-text').innerHTML = '✅ Doğru! Tüm boşluklar doğru dolduruldu.';
                }
                ...
                const wrongAnswerText = wrongAnswers.map(w => `Kullanıcı: "${w.user}", Doğru: "${w.correct}"`).join('; ');
                const correctAnswerText = correctAnswers.join(', ');
                ...
                if (explanationBox) {
                    ...
                    explanationBox.querySelector('.explanation-text').innerHTML = 
                        `❌ ${correctCount}/${correctAnswers.length} boşluk doğru. Doğru cevaplar: ${correctAnswerText}`;
        ...
        async function getAIRecommendationForFillBlank(questionIndex, wrongAnswerText, correctAnswerText, explanationBox) {
            try {
                if (!window.GroqAPI && !window.GeminiAPI) {
                    console.warn('⚠️ Groq API mevcut değil');
                    return;
                }
                ...
                if (!api || typeof api.generateVideoRecommendation !== 'function') {
                    console.error('❌ Groq API veya generateVideoRecommendation fonksiyonu bulunamadı');
                return;
            }
                            ...
                            <div class="flex-1">
                                ...
                                <div class="bg-white p-3 rounded-lg border border-purple-200 mb-3">
                                    <p class="text-sm font-semibold text-purple-800 mb-1">
                                        <i class="fas fa-video mr-2"></i>${recommendation.recommendedVideoTitle || 'Önerilen Video'}
                                    </p>
                                ...
                                <button onclick="goToRecommendedVideo('${recommendation.recommendedVideoId}')" 
                                    ...
                                    <i class="fas fa-play-circle mr-2"></i>Bu Videoyu İzle
                    </button>
                        </div>
                    </div>
                `;
                    ...
                    if (recommendation.error && recommendation.error.includes('Rate limit')) {
                        ...
                        feedbackDiv.innerHTML = `
                            <p class="text-sm text-red-700">
                                ...
                                <strong>API Limit Aşıldı:</strong> Çok fazla istek gönderildi. Lütfen birkaç dakika sonra tekrar deneyin.
                            </p>
                            <p class="text-xs text-red-600 mt-2">
                                ${recommendation.feedback || 'Bu konuyu tekrar gözden geçirmenizi öneririz.'}
                            </p>
                        `;
                } else {
                ...
                errorDiv.innerHTML = `
                    <p class="text-sm text-red-700">
                        <i class="fas fa-exclamation-triangle text-red-600 mr-2"></i>
                        <strong>Hata:</strong> AI önerisi alınamadı. Lütfen daha sonra tekrar deneyin.
                    </p>
                    <p class="text-xs text-red-600 mt-2">
        ...
        function selectAnswer(questionIndex, selectedOption, correctAnswer) {
            ...
            const studentAnswer = String.fromCharCode(65 + selectedOption);
            ...
            if (isCorrect) {
                ...
                explanationBox.classList.remove('border-red-200', 'bg-red-50');
                explanationBox.classList.add('border-green-200', 'bg-green-50');
                explanationText.innerHTML = '✅ Doğru cevap! ' + (question.explanation || 'Tebrikler!');
                
                // Save correct answer
                saveQuestionAnswer(questionIndex, question.question, studentAnswer, correctAnswerText, true, question.explanation);
                ...
                explanationBox.classList.remove('border-green-200', 'bg-green-50');
                explanationBox.classList.add('border-red-200', 'bg-red-50');
                explanationText.innerHTML = '❌ Yanlış cevap. Doğru cevap: ' + correctAnswerText + '. ' + (question.explanation || '');
                
                // Save wrong answer
                saveQuestionAnswer(questionIndex, question.question, studentAnswer, correctAnswerText, false, question.explanation);
        ...
        async function getAIRecommendation(questionIndex, selectedOption, correctAnswer, explanationBox) {
            try {
                ...
                if (!question || !question.options) {
                    console.error('❌ Soru veya seçenekler bulunamadı');
                    loadingDiv.remove();
                    return;
                }
                
                const wrongAnswerText = question.options[selectedOption] || 'Seçilen cevap';
                const correctAnswerText = question.options[correctAnswer] || 'Doğru cevap';
                
                console.log('🤖 AI önerisi isteniyor:', {
        ...
        function goToRecommendedVideo(videoId) {
            ...
            alert('Önerilen video bulunamadı.');
        }
        ...
        async function updateVideoProgress() {
            ...
            console.log(`📊 Video ilerleme güncellendi: ${correctAnswers}/${totalQuestions} = ${progressPercent}%`);
            console.log(`📊 Modül ilerleme: ${totalModuleCorrect}/${totalModuleQuestions} = ${moduleProgress}%`);
        }
        
        // Save question answer to Firebase
        async function saveQuestionAnswer(questionIndex, questionText, studentAnswer, correctAnswer, isCorrect, explanation) {
            try {
                const userEmail = localStorage.getItem('userEmail');
                ...
                await window.DB.save('videoQuestionAnswers', answerData);
                console.log('✅ Soru cevabı kaydedildi:', answerData);
                
            } catch (error) {
                console.error('❌ Soru cevabı kaydetme hatası:', error);
            }
        }
        ...
        async function saveProgressToFirebase(moduleProgress, videoId, videoProgress) {
            ...
            if (videoId && videoProgress) {
                progressData.videoProgress[videoId] = {
                    ...
                    questions: videoProgress.questions || [],
                    updatedAt: new Date().toISOString()
                };
            }
                ...
                console.log('✅ İlerleme Firebase\'e kaydedildi');
        ...
        function saveVideoNote(noteText) {
            ...
            const note = {
                ...
                moduleTitle: module?.title || 'Bilinmeyen Modül'
            };
        ...
        function displaySavedNotes() {
            ...
            if (allNotes.length === 0) {
                savedNotesContainer.innerHTML = '<p class="text-gray-500 text-sm">Henüz not yok</p>';
                    return;
                }
            ...
            if (allNotes.length > 3) {
                ...
                moreNotesElement.innerHTML = `
                    <a href="my-notes.html" class="text-cyber text-sm hover:underline">
                        +${allNotes.length - 3} daha fazla not görüntüle
                    </a>
                `;
