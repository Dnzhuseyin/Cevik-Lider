...
<head>
    ...
    <title>Video Yönetimi - Çevik Lider Admin</title>
...
<body class="bg-gray-50 min-h-screen">
    ...
    <main class="main-content p-4 md:p-8 w-full min-h-screen">
        <div class="max-w-7xl mx-auto">
            ...
            <header class="mb-8">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex gap-3">
                        <button onclick="createTestVideo()" class="bg-orange-500 text-white px-4 py-2 rounded-lg flex items-center hover:bg-orange-600">
            ...
            <!-- Video Upload Section - YENİ TASARIM -->
            <div class="bg-white rounded-xl shadow-sm p-6 mb-8 border border-gray-100">
                ...
                <form id="videoForm" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" id="videoTitle" required
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyber focus:border-transparent"
                                       placeholder="Örn: Kriptoloji Temelleri">
                        </div>
                        <div>
                        ...
                        <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Süre (dk) *</label>
                    ...
                    <div class="bg-blue-50 p-4 rounded-lg border border-blue-200">
                        <h3 class="font-semibold mb-4 text-blue-800">2. Video Soruları (Opsiyonel)</h3>
                        <p class="text-sm text-gray-600 mb-4">Video için farklı tipte sorular ekleyebilirsiniz.</p>
                        ...
                        <div class="flex gap-2 mb-4">
                            <button type="button" onclick="addNewVideoQuestion('multiple-choice')" class="flex-1 bg-primary text-white px-4 py-2 rounded-lg hover:bg-primary/90">
                                <i class="fas fa-list mr-2"></i>Çoktan Seçmeli
                            </button>
                            <button type="button" onclick="addNewVideoQuestion('open-ended')" class="flex-1 bg-accent text-white px-4 py-2 rounded-lg hover:bg-accent/90">
                                <i class="fas fa-comment-alt mr-2"></i>Açık Uçlu
                            </button>
                            <button type="button" onclick="addNewVideoQuestion('fill-blank')" class="flex-1 bg-secondary text-white px-4 py-2 rounded-lg hover:bg-secondary/90">
                                <i class="fas fa-edit mr-2"></i>Boşluk Doldurma
                        </button>
                    </div>
    ...
    <script>
        ...
        async function updateStats() {
            try {
                ...
                console.log('📊 İstatistikler güncellendi:', { totalVideos, totalQuestions });
        ...
        async function deleteVideo(videoId) {
            ...
            try {
                console.log('🗑️ Video siliniyor:', videoId);
                
                // Get existing video first
                const existingVideoResult = await window.DB.load('coordinatorVideos', videoId);
        ...
        async function generateAIQuestionsForVideo(videoId, videoTitle) {
            try {
                console.log('🤖 AI soru üretimi başlatılıyor:', videoId, videoTitle);
                
                const api = window.GroqAPI || window.GeminiAPI;
                if (!api) {
                    alert('AI soru üretimi için Groq API gerekli!');
                    return;
                }
                ...
                if (questions && questions.length > 0) {
                    ...
                    alert('AI soru üretimi başarısız oldu.');
                }
                
            } catch (error) {
                console.error('❌ AI soru üretimi hatası:', error);
                alert('AI soru üretimi sırasında hata oluştu: ' + error.message);
            }
        }
        ...
        async function addManualQuestionsToVideo(videoId, videoTitle) {
            try {
                console.log('✏️ Manuel soru ekleme başlatılıyor:', videoId, videoTitle);
                ...
                alert(`"${videoTitle}" videosuna manuel soru ekleme moduna geçildi.`);
                
            } catch (error) {
                console.error('❌ Manuel soru ekleme hatası:', error);
                alert('Manuel soru ekleme sırasında hata oluştu: ' + error.message);
            }
        }
        ...
        function addNewVideoQuestion(questionType = 'multiple-choice') {
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        <textarea class="question-text w-full p-2 border border-gray-300 rounded-lg" rows="2" placeholder="${
                            questionType === 'fill-blank' ? 'Boşlukları ___ ile belirtin. Örn: Kriptoloji ___ bilimi olarak tanımlanır.' : 'Sorunuzu yazın...'
                        }" required></textarea>
                    </div>
            `;
            ...
            if (questionType === 'multiple-choice') {
                questionHTML += `
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block text-sm text-gray-600 mb-1">A) Seçenek</label>
                            <input type="text" class="option-a w-full p-2 border border-gray-300 rounded-lg" placeholder="A seçeneği" required>
                        </div>
                        <div>
                            <label class="block text-sm text-gray-600 mb-1">B) Seçenek</label>
                            <input type="text" class="option-b w-full p-2 border border-gray-300 rounded-lg" placeholder="B seçeneği" required>
                        </div>
                        <div>
                            <label class="block text-sm text-gray-600 mb-1">C) Seçenek</label>
                            <input type="text" class="option-c w-full p-2 border border-gray-300 rounded-lg" placeholder="C seçeneği" required>
                        </div>
                        <div>
                            <label class="block text-sm text-gray-600 mb-1">D) Seçenek</label>
                            <input type="text" class="option-d w-full p-2 border border-gray-300 rounded-lg" placeholder="D seçeneği" required>
                        </div>
                    </div>
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block text-sm text-gray-600 mb-1">Doğru Cevap</label>
                ...
                questionHTML += `
                    <div>
                        <label class="block text-sm text-gray-600 mb-1">Örnek Cevap Anahtar Kelimeleri (virgülle ayırın)</label>
                        <input type="text" class="answer-keywords w-full p-2 border border-gray-300 rounded-lg" 
                               placeholder="Örn: şifreleme, güvenlik, anahtar" required>
                        <p class="text-xs text-gray-500 mt-1">AI bu anahtar kelimeleri kullanarak cevapları değerlendirecek</p>
                    </div>
                    <div>
                        <label class="block text-sm text-gray-600 mb-1">Zorluk</label>
                        <select class="question-difficulty w-full p-2 border border-gray-300 rounded-lg">
                ...
                questionHTML += `
                    <div>
                        <label class="block text-sm text-gray-600 mb-1">Doğru Cevaplar (virgülle ayırın, boşluk sırasına göre)</label>
                        <input type="text" class="fill-blank-answers w-full p-2 border border-gray-300 rounded-lg" 
                               placeholder="Örn: şifreleme, kriptoloji" required>
                        <p class="text-xs text-gray-500 mt-1">Her boşluk için doğru cevabı yazın</p>
                    </div>
                    <div>
                        <label class="block text-sm text-gray-600 mb-1">Zorluk</label>
                        <select class="question-difficulty w-full p-2 border border-gray-300 rounded-lg">
            ...
            questionHTML += `
                    <div>
                        <label class="block text-sm text-gray-600 mb-1">Açıklama (Opsiyonel)</label>
                        <textarea class="question-explanation w-full p-2 border border-gray-300 rounded-lg" rows="2" placeholder="Cevabın açıklaması..."></textarea>
                    </div>
                </div>
            `;
            ...
            console.log(`✅ ${questionType} tipi soru ${questionNumber} eklendi`);
        }
        ...
        document.addEventListener('DOMContentLoaded', async function() {
            ...
            if (!window.DB) {
                console.log('⏳ Firebase DB bekleniyor...');
                let waitAttempts = 0;
                while (!window.DB && waitAttempts < 10) {
                    await new Promise(resolve => setTimeout(resolve, 500));
                    waitAttempts++;
                    console.log(`⏳ Firebase DB bekleme denemesi: ${waitAttempts}/10`);
                }
                
                if (!window.DB) {
                    console.error('❌ Firebase DB sistemi 5 saniye içinde yüklenmedi!');
                    return;
                }
            }
            ...
            if (!window.DB.isFirebaseReady) {
                console.log('⏳ Firebase hazır olması bekleniyor...');
                let readyAttempts = 0;
                while (!window.DB.isFirebaseReady && readyAttempts < 10) {
                    await new Promise(resolve => setTimeout(resolve, 500));
                    readyAttempts++;
                    console.log(`⏳ Firebase ready bekleme denemesi: ${readyAttempts}/10`);
                }
                
                if (!window.DB.isFirebaseReady) {
                    console.error('❌ Firebase 5 saniye içinde hazır olmadı!');
                    return;
                }
            }
            
            console.log('✅ Firebase hazır, coordinator content yükleniyor...');
                    ...
                    questionDivs.forEach((div, index) => {
                        ...
                        if (questionType === 'multiple-choice') {
                            ...
                            if (optionA && optionB && optionC && optionD && correctAnswer !== '') {
                                ...
                                console.log(`✅ Çoktan seçmeli soru ${index + 1} eklendi`);
                            }
                        } else if (questionType === 'open-ended') {
                            ...
                            if (keywords) {
                                ...
                                questions.push(questionData);
                                console.log(`✅ Açık uçlu soru ${index + 1} eklendi`);
                            }
                        } else if (questionType === 'fill-blank') {
                            ...
                            if (answers) {
                                ...
                                questions.push(questionData);
                                console.log(`✅ Boşluk doldurma soru ${index + 1} eklendi`);
                            }
                        }
                    });
                    
                    console.log(`📝 Toplam ${questions.length} soru eklendi`);
        ...
        function resetVideoForm() {
            ...
            console.log('✅ Video formu sıfırlandı');
        }
        ...
        async function loadVideos() {
            try {
                ...
                const videosResult = await window.DB.load('coordinatorVideos');
                const allVideos = (videosResult && videosResult.success) ? videosResult.data : [];
                
                // Filter out deleted videos for coordinator view
                const videos = allVideos.filter(video => video.status !== 'deleted');
                const videosList = document.getElementById('videos-container');
                
                console.log('📹 Toplam video sayısı:', allVideos.length, 'Aktif video sayısı:', videos.length);
                ...
                if (videosContainer) {
                    videosContainer.innerHTML = '<p class="text-red-500 text-center py-8">Videolar yüklenirken hata oluştu.</p>';
                }
            }
        }
        ...
        async function loadModules() {
            try {
                ...
                if (!window.DB || !window.DB.isFirebaseReady) {
                    console.error('❌ Firebase hazır değil, modüller yüklenemiyor');
                    return;
                }
                ...
                try {
                    ...
                    if (modulesResult && modulesResult.success && modulesResult.data) {
                        ...
                        console.log('✅ loadModules() ile modüller yüklendi:', modules.length);
                    }
                } catch (err) {
                    console.warn('⚠️ loadModules() başarısız:', err);
                }
                ...
                if (modules.length === 0) {
                    try {
                        ...
                        if (modulesResult && modulesResult.success && modulesResult.data) {
                            modules = modulesResult.data;
                            console.log('✅ load("modules") ile modüller yüklendi:', modules.length);
                        }
                    } catch (err) {
                        console.warn('⚠️ load("modules") başarısız:', err);
                    }
                }
                ...
                if (moduleSelect) {
                    ...
                    if (modules && modules.length > 0) {
                        ...
                        try {
                            ...
                            if (modulesResult && modulesResult.success && modulesResult.data) {
                                modules = modulesResult.data;
                                console.log('✅ Örnek modüller oluşturulduktan sonra yüklendi:', modules.length);
                            }
                        } catch (err) {
                            console.error('❌ Örnek modüller oluşturulduktan sonra da yüklenemedi:', err);
                        }
                        
                        if (modules.length === 0) {
                            console.error('❌ Modüller hala boş!');
                            moduleSelect.innerHTML = '<option value="">Modül bulunamadı</option>';
                        return;
                        }
                    }
                } else {
        ...
        async function createSampleModules() {
            try {
                ...
                for (let i = 0; i < sampleModules.length; i++) {
                    ...
                    console.log(`📚 Modül ${moduleId} kaydedildi:`, result.success ? 'Başarılı' : 'Başarısız');
                }
        ...
        async function createTestVideo() {
            try {
                console.log('🧪 Test videosu oluşturuluyor...');
                
                const testVideo = {
                    title: 'Test Video - Silinecek',
                    moduleId: '1',
                    youtubeUrl: 'https://www.youtube.com/watch?v=dQw4w9WgXcQ',
                    youtubeVideoId: 'dQw4w9WgXcQ',
                    description: 'Bu test videosu silme işlemini test etmek için oluşturuldu',
                    duration: '3:32',
                    difficulty: 'easy',
                    status: 'active',
                    questions: [],
                ...
                const result = await window.DB.save('coordinatorVideos', testVideo);
                if (result && result.success) {
                    console.log('✅ Test videosu oluşturuldu');
                    alert('✅ Test videosu oluşturuldu! Şimdi videoları yeniden yükleyin.');
                    await loadVideos();
                    await updateStats();
                } else {
                    throw new Error('Test videosu oluşturulamadı');
                }
                
            } catch (error) {
                console.error('❌ Test videosu oluşturma hatası:', error);
                alert('❌ Test videosu oluşturulurken hata: ' + error.message);
            }
            try {
